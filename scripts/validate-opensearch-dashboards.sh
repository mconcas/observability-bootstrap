...
HTTP_CODE=$(curl -so /dev/null -w '%{http_code}' \
  "${OS_AUTH[@]}" "http://localhost:5601/api/status" 2>/dev/null || echo "000")
[ "$HTTP_CODE" = "200" ] \
  && pass "API responding (HTTP $HTTP_CODE)" \
  || fail "API returned HTTP $HTTP_CODE"

# 3. Observability workspace exists
WS=$(curl -s "${OS_AUTH[@]}" -H "osd-xsrf: true" -H "Content-Type: application/json" \
...
DC=$(curl -s "${OS_AUTH[@]}" -H "osd-xsrf: true" \
...
for pattern in "otel-v1-apm-span*" "logs-otel-v1*" "otel-v2-apm-service-map*"; do
  FOUND=$(curl -s "${OS_AUTH[@]}" -H "osd-xsrf: true" \
